...
⚠️ vLLM 中 `MLAAttentionSpec` 实际上**映射到 `FullAttentionManager`**，并没有独立的 `MLAAttentionManager` 类。MLA（Multi-head Latent Attention）场景复用全注意力管理器的前缀缓存逻辑，同时通过低秩压缩减少 KV 缓存显存占用。
...
随机滑动窗口注意力管理器（Randomized Sliding Window Attention），用于部分注意力层的滑动窗口缓存管理。

### `SinkFullAttentionManager`

Sink 全注意力管理器，支持保留初始 token（attention sink）的全注意力缓存策略。
